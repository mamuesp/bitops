...
/*
 * Helper for reading the bit 'bitNum' from 1-byte register `reg` from a 
 * device at address `addr`.
 * In case of success returns a value: representing the bit (0 or !).
 *
 * In case of error returns -1.
 */          
...
/*
 * Helper for reading the bit 'bitNum' from 2-byte register `reg` from a 
 * device at address `addr`.
 * In case of success returns a value: representing the bit (0 or !).
 *
 * In case of error returns -1.
...
/*
 * Helper for reading some bits described by 'bitStart' and 'length' from 1-byte
 * register `reg` from a device at address `addr`.
 * In case of success returns a value: representing the bits
 *
 * In case of error returns -1.
 */          
...
/*
 * Helper for reading some bits described by 'bitStart' and 'length' from 2-byte
 * register `reg` from a device at address `addr`.
 * In case of success returns a value: representing the bits
 *
 * In case of error returns -1.
...
/*
 * Helper for reading some bits in a 1-byte value described by 'bitStart' and
 * 'length' from a device at address `addr` without register.
 * In case of success returns a value: representing the bits
 *
 * In case of error returns -1.
 */          
...
/*
 * Helper for reading some bits in a 2-byte value described by 'bitStart' and
 * 'length' from a device at address `addr` without register.
 * In case of success returns a value: representing the bits
 *
 * In case of error returns -1.
...
/*
 * Helper for setting the bit 'bitNum' to 'value' (0 or 1) in a 1-byte register
 * `reg` to a device at address `addr`.
 * Returns `true` in case of success, `false` otherwise.
 */
...
/*
 * Helper for setting the bit 'bitNum' to 'value' (0 or 1) in a 2-byte register
 * `reg` to a device at address `addr`.
 * Returns `true` in case of success, `false` otherwise.
 */
...
/*
 * Helper for setting the bits described by 'bitStart' and 'length' to 'value' 
 * in a 1-byte register `reg` to a device at address `addr`.
 * Returns `true` in case of success, `false` otherwise.
 */
...
/*
 * Helper for setting the bits described by 'bitStart' and 'length' to 'value' 
 * in a 2-byte register `reg` to a device at address `addr`.
 * Returns `true` in case of success, `false` otherwise.
 */
...
/*
 * Helper for setting the bits described by 'bitStart' and 'length' to 'value' 
 * in a 1-byte write to a device at address `addr` without register.
 * Returns `true` in case of success, `false` otherwise. With 'stop' set to true,
 * the I2C bus will be released after the commands.
 */
...
/*
 * Helper for setting the bits described by 'bitStart' and 'length' to 'value' 
 * in a 2-byte write to a device at address `addr` without register.
 * Returns `true` in case of success, `false` otherwise. With 'stop' set to true,
 * the I2C bus will be released after the commands.
 */
...
/*
 * Helper for calculating the result for 'b' setting only the bits described by
 * 'bitStart' and 'length' and 'value' returning a 1-byte value.
 */
uint8_t mgos_calc_bits_b(uint8_t b, uint8_t bitStart, uint8_t length, uint8_t value);

/*
 * Helper for calculating the result for 'w' setting only the bits described by
 * 'bitStart' and 'length' and 'value' returning a 2-byte value.
 */
